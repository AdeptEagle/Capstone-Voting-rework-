...
async function checkTemplatePositions() {
  try {
    ...
    for (const template of DEFAULT_BALLOT_TEMPLATES) {
      ...
      const dbTitles = templateData.positions?.map((p: any) => p.positionTitle) || [];
      
      const missingPositions = expectedTitles.filter(title => !dbTitles.includes(title));
      const extraPositions = dbTitles.filter((title: string) => !expectedTitles.includes(title));
    ...
    // Check if there are any existing positions in the database
    ...
    // Check for any ballots created from templates

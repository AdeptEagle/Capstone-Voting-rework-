...
async function diagnoseTemplateIssue() {
  try {
    ...
    // 1. Check what templates exist in database
    ...
    for (const defaultTemplate of DEFAULT_BALLOT_TEMPLATES) {
      ...
      dbPositions.forEach((pos: any, index: number) => {
        console.log(`   ${index + 1}. ${pos.positionTitle} (Order: ${pos.displayOrder}, Required: ${pos.isRequired}, Vote Limit: ${pos.voteLimit})`);
      });

      // Find missing positions
      const expectedTitles = defaultTemplate.data.positions.map(p => p.positionTitle);
      const dbTitles = dbPositions.map((p: any) => p.positionTitle);
      
      const missing = expectedTitles.filter(title => !dbTitles.includes(title));
      const extra = dbTitles.filter((title: string) => !expectedTitles.includes(title));
    ...
    // 3. Check if there are any ballots created from templates
    ...
    // 4. Check all positions in database
    ...
    // 5. Check for any template-related issues
    console.log('\n5️⃣ Checking for template-related issues...');
    
    // Check if there are any templates with missing data
    ...
    // Check for duplicate position titles
    const allPositionTitles = allPositions.map(p => p.Position_Title);
    const duplicateTitles = allPositionTitles.filter((title, index) => 
      allPositionTitles.indexOf(title) !== index
    );
